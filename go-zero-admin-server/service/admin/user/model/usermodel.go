...
func (d defaultUserModel) GetUserByUsername(username string) (*User, error) {
	var user *User
	err := d.conn.Model(&User{}).Preload("Role").Where("username = ?", username).First(&user).Error
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (d defaultUserModel) GetUserByID(id uint) (*User, error) {
	var user *User
	err := d.conn.Model(&User{}).Preload("Role").Where("id = ?", id).First(&user).Error
	if err != nil {
		return nil, err
	}
	return user, nil
...
func (d defaultUserModel)  AddUser(user *User) error{
...
func (d defaultUserModel)  UpdateUser(user *User) error{
...
func (d defaultUserModel) 	DelUser(user *User) error{
